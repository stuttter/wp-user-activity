# Support

Use the [WordPress.org support forum](https://wordpress.org/support/plugin/wp-user-activity/)
for installation and usage questions. Use the GitHub issue tracker for
reproducible defects and narrowly scoped enhancements, and GitHub Discussions
for broader development conversations.

Security vulnerabilities must follow `SECURITY.md` and must not be reported in
a public issue.
